...
export class OutfitAddPage implements OnInit {
  ...
  saveNewOutfit(title, desc) {
    if (title.value === "") {
      ...
      this.resultSave = this.http.post('http://34.72.164.39/outfits/new-outfit/save', data)
        .subscribe(res => {
          this.resultSave = res;
        });
  ...
  onUpload() {
    if (this.cardImageBase64 === "") {
      ...
      this.predictionRes = this.http.post('http://34.72.164.39/outfits/new-outfit/predict', data)
        .subscribe(res => {
